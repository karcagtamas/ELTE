...
* inversion H. subst. reflexivity.
* inversion H1. subst. assert (s' = s'0). apply IHeval_bigstep1. assumption. subst. apply IHeval_bigstep2. assumption.
* inversion H1. subst.
** apply IHeval_bigstep. assumption.
** subst. rewrite H in H7. inversion H7.
* inversion H1. subst.
** rewrite H in H7. inversion H7.
** subst. apply IHeval_bigstep. assumption.
* inversion H2. subst.
** assert (s' = s'0). apply IHeval_bigstep1. assumption. subst. apply IHeval_bigstep2. assumption.
** subst. rewrite H in H7. inversion H7.
* inversion H0. subst.
...
Definition simassign_sugar (x1 x2 : string) (a1 a2 : aexp) : cmd := x1 ::= a1 ;; x2 ::= a2.
...
apply eval_if_true. assumption. eapply eval_seq. exact H3. assumption.
...
(* FELADAT *)
Proof.
intro. Check (H X Y 1 X).
assert (Equiv (csimassign X Y 1 X) (simassign_sugar X Y 1 X)). apply H.
Check (H0 empty).
destruct (H0 empty (update Y (aeval X empty) (update X (aeval 1 empty) empty))).
assert (| simassign_sugar X Y 1 X, empty | -=> update Y (aeval X empty) (update X (aeval 1 empty) empty)).
apply H1. eapply eval_simassign.
Admitted.
...
intros.
split.
* intro. inversion H. subst. apply eval_simassign.
Admitted.