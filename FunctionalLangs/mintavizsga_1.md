...
Írd meg a következő instance-okat! **(4 pont)**
...
Definiálj egy függvényt, ami egy `BiList`-nek az első legfeljebb `n` darab `ConsSnoc` konstruktorát veszi, a többit eldobja! **(1 pont)**
...
Definiálj egy függvényt, ami egy `BiList`-et listává alakít! Egy `ConsSnoc` konstruktor első paramétere a lista elejére, az utolsó paramétere a lista végére kerüljön! **(2 pont)**
...
Definiálj egy függvényt, ami megfordítja az elemek sorrendjét egy `BiList`-ben! **(2 pont)**
...
Definiálj egy függvényt, ami egy `BiList`-et megcímkéz _preorder_ bejárási sorrendben, ahol
...
Adjuk a szintaxishoz a `Null :: Exp`, `AssignIfNull :: String -> Exp -> Statement` és `IsNull :: Exp -> Exp` konstruktorokat! **(1 pont)**
...
Egészítsük ki a `Val` típust egy `VNull :: Val` konstruktorral! **(1 pont)**
