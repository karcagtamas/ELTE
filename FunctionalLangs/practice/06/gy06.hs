...
io1 :: IO ()
io1 = do
  putStrLn "Give a text:"
  s <- getLine
  let c = length $ filter (== 'a') s
  putStrLn $ "Count of 'a' characters: " <> show c
...
instance Traversable STree where
  ...
  sequenceA (SNode a x b) = SNode <$> sequenceA a <*> x <*> sequenceA b
...
printLines = undefined -- traverse $ \a -> do
--  print a
--  return ()
...
instance Traversable Pit where
  ...
  sequenceA (PLeft a p) = PLeft <$> a <*> sequenceA p
  sequenceA (PRight p a) = PRight <$> sequenceA p <*> a

instance Traversable RoseTree where
  sequenceA :: (Applicative t) => RoseTree (t a) -> t (RoseTree a)
  sequenceA (RoseNode a o) = RoseNode <$> a <*> traverse sequenceA o
...
instance (Traversable f, Traversable g) => Traversable (Compose f g) where
  ...
  sequenceA (Compose f) = undefined -- Compose <$> sequenceA f
