...
pGt :: Parser Exp
pGt = nonAssoc Gt pLt (char' '>')

pLte :: Parser Exp
pLte = nonAssoc Lte pGt (string' "<=")

pGte :: Parser Exp
pGte = nonAssoc Gte pLte (string' ">=")

pAnd :: Parser Exp
pAnd = rightAssoc And pGte (string' "&&")
...
pNeq :: Parser Exp
pNeq = nonAssoc (\a b -> Not (Eq a b)) pEq (string' "/=")
...
statement =
  Assign <$> identifier <*> (string' ":=" *> pExp)
    ...
    <|> If <$> (string' "if" *> pExp) <*> (string' "then" *> program) <*> ([] <$ string' "end")
...
evalExp (Var a) env = do
  case lookup a env of
    (Just v) -> Right $ v
    _ -> Left "Undefined"
evalExp (Lt a b) env = do
  a <- evalExp a env
  b <- evalExp b env
  case (a, b) of
    (VInt a, VInt b) -> Right $ VBool (a < b)
    _ -> Left "Type error"
evalExp (Gt a b) env = do
  a <- evalExp a env
  b <- evalExp b env
  case (a, b) of
    (VInt a, VInt b) -> Right $ VBool (a > b)
    _ -> Left "Type error"
evalExp (Lte a b) env = do
  a <- evalExp a env
  b <- evalExp b env
  case (a, b) of
    (VInt a, VInt b) -> Right $ VBool (a <= b)
    _ -> Left "Type error"
evalExp (Gte a b) env = do
  a <- evalExp a env
  b <- evalExp b env
  case (a, b) of
    (VInt a, VInt b) -> Right $ VBool (a >= b)
...
evalStatement (Assign var exp) = do
  ...
  put (updateEnv var v env)
evalStatement (While exp prog) = do
  env <- get
  (VBool c) <- evalEither $ evalExp exp env
  if c
    then do
...
evalStatement (If exp yes no) = do
  env <- get
  (VBool c) <- evalEither $ evalExp exp env
  inNewScope $ if c then evalProgram yes else evalProgram no
