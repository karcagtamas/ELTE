...
csövet is az eredményben! **(1 pont)**
...
**Segítség:** A `words` függvény a `Data.List` modulból fel tud bontani egy
...
Adjuk a szintaxishoz a `EmptyStack :: Exp`, `PopInto :: String -> String -> Statement`, `Push :: String -> Exp -> Statement` és `IsEmpty :: Exp -> Exp` konstruktorokat! **(1 pont)**
...
Egészítsd ki a `Val` típust egy `VStack :: [Val] -> Val` konstruktorral. **(1 pont)**
...
- A `Push` állítás szúrja be a paraméterül kapott változó **elejére** a második
