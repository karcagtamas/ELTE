...
`Cycle` konstruktor paramétere nem üres) **(1 pont)**
...
listaelemek beszúrása a `Cycle` konstruktor elé kerüljön! **(2 pont)**
...
ismétlődő része lesz (A `Cycle` konstruktor paramétere)! **(2 pont)**

Feltehetjük, hogy a bemenet mindig helyes és a számok space-el vannak elválasztva.

**Segítség:** A `read` függvény segítségével tudunk stringből számot csinálni.

**Segítség:** Az `unwords` függvény a `Data.List` modulból fel tud bontani egy string-et spacenként.
...
bemeneti paraméter `Cycle` konstruktorában! **(3 pont)**
...
Adjuk a szintaxishoz az `Index :: Exp -> Exp ->  Exp`, `List :: [Exp] -> Exp` és `AssignAt :: String -> Exp -> Exp -> Statement` konstruktorokat **(1 pont)**
...
Egészítsd ki a `Val` típust egy `VList :: [Val] -> Val` konstruktorral. **(1 pont)**
